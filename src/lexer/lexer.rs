...
impl<'a> Lexer<'a> {
    ...
    pub fn next_token(&mut self) -> Result<Option<Token>, LexerError> {
        ...
        match current {
            ...
            operator @ ('=' | '+' | '-' | '*' | '/' | '^' | '.') => {
                self.advance();
                Self::tokenize_operator(operator, start_line, start_column).map(Some)
            }

            delimiter @ ('(' | ')' | '[' | ']' | '<' | '>' | ',' | ':' | '|') => {
                self.advance();
                Self::tokenize_delimiter(delimiter, start_line, start_column).map(Some)
            }
    ...
    fn tokenize_delimiter(delimiter: char, line: u32, column: u32) -> Result<Token, LexerError> {
    ...
    fn tokenize_operator(operator: char, line: u32, column: u32) -> Result<Token, LexerError> {
